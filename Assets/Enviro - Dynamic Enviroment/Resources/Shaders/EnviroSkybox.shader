	...
	SubShader 
	{
    	...
    	Pass 
    	{
			...
			float4 frag(v2f i) : SV_Target
			{
			   ...
			   float3 starsMap = texCUBE(_Stars, i.sunPos.xyz);
			   float3 stars = saturate(starsMap * starsMap);
			   float starsBehindMoon =  1 - clamp((moonBright * 2),0,1);

			   stars = clamp((stars * fadeStar) * starsBehindMoon,0,4);

               float3 skyScattering = (scattering + sunDisk) + (skyFinalize + stars + moonBright);
